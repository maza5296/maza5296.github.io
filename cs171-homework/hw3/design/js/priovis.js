...
PrioVis.prototype.initVis = function(){
    ...
    this.svg.append("g")
      ...
      .append("text")
        ...
        .html("dispribution of priorities (in %)");
      ...
      .append("text")
        ...
        .attr("dy", "20")
        .style("text-anchor", "end")
        .html("left: brush selection, right: average");
...
PrioVis.prototype.updateVis = function(){
    ...
    var range = (d3.extent(this.displayData, function(d) { return d.count; })).concat(d3.extent(this.averageData, function(d) {return d.count}))
    this.y.domain(d3.extent(range));
    ...
    var bar = this.svg.selectAll(".bar2")
      .data(this.averageData, function(d) { return d.type; });

    var bar_aver_enter = bar.enter().append("g");

    // Append a rect and a text only for the Enter set (new g)
    bar_aver_enter.append("rect")

    bar
      .attr("class", "bar2")
      .attr("transform", function(d, i) { 
          return "translate(" + (that.x(d.type)+17) +",0)"; })
...
PrioVis.prototype.wrangleData = function(_filterFunction){
    ...
    res.map(function(d, i){
        ...
        count: (res[i]/count)*100
      }
        that.averageData.push(tmp);
    });
...
PrioVis.prototype.filterAndAggregate = function(_filter){
    ...
    var count = 0;
    this.filteredData.map(function(d){
        count = count + d.count
    })
